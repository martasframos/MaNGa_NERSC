...
####################################################################################################################
####################################################################################################################
f_names=['July_Age_bin_re.csv', 'Bin_6.csv']

path='/global/cscratch1/sd/marta_r/2021-10-10/_Multiprocessing/July_fitting_files/July_Age_bin_re.csv'
...
properties=[]
for j in range(30):
    df = pd.read_csv("/global/homes/m/marta_r/MaNGa_files/July_Age_bin_test.csv")
    cd=random.randint(0,df[df.columns[0]].count())
    if 'Unnamed: 0' in df.columns:
        df = df.drop('Unnamed: 0', axis=1)
        df.to_csv("/global/homes/m/marta_r/MaNGa_files/July_Age_bin_test.csv")
    for p in range (df[df.columns[0]].count()):
        ...
        if cd <= (df[df.columns[0]].count())-1 and df.loc[cd, 'Validation'] == 0:
            ...
            df.to_csv("/global/homes/m/marta_r/MaNGa_files/July_Age_bin_test.csv", index=True)
            z,ifu,ratio=df.loc[cd, 'redhsift'], df.loc[cd, 'Spectra'],df.loc[cd, 'ratio']
            rad,ba,phi=df.loc[cd,'radius'], df.loc[cd,'nsa_elpetro_ba'], df.loc[cd,'nsa_elpetro_phi'] #radius changes to Re in the Re fitting
...
os.environ['SAS_BASE_DIR'] = '/global/cfs/cdirs/sdss/data/sdss'
radius=properties[0][3] #Replace with i_obs when fitting on nersc
...
dataframe.to_csv('September_fit_re.csv', mode='a', header=False)
